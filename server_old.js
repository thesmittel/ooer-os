...
let users =
    JSON.parse(fs.readFileSync("./server/users/userdata.json").toString())
        .users;
...
app.get("/media/desktopicons/:app", (req, res) => {
    if (req.params.app.match(/^\d{12}$/g)) {
        res.sendFile(
            __dirname + "/server/applications/custom/" + req.params.app +
                "/icon.png",
        );
        return;
    }
    res.sendFile(
        __dirname + "/server/applications/system/" + req.params.app +
            "/icon.png",
    );
});
...
function authReq(socket, data) {
    ...
    switch (data.req) {
        ...
        case "signup": {
            ...
            const username = Auth.signupCheckUsernameAvailable(
                socket,
                users,
                data.data.username,
            );
            const email = Auth.signupCheckEmailRegistered(
                socket,
                users,
                data.data.email,
            );
            const pwmatch = Auth.signupCheckPasswordMatch(socket, data.data);
            const pwreq = Auth.signupCheckPasswordRequirements(
                socket,
                data.data,
            );
    ...
    switch (data.req) {
        ...
        case "terminal_req":
            socket.emit("App", {
                ...
                data: { res: "test", fp: data.data.fp, id: data.data.id },
            });
    }
}
...
io.on("connection", (socket) => {
    ...
    socket.on("disconnect", () => {
        Auth.removeSocket(socket);
    });
    socket.on("Auth", (data) => {
        authReq(socket, data);
    });
    socket.on("Client", (data) => {
        clientReq(socket, data);
    });
    socket.on("System", (data) => {
        systemReq(socket, data);
    });
    socket.on("App", (data) => {
        appReq(socket, data);
    });
