...
function initialiseDesktop() {
  document.body.append(
    ...
    Util.create({
      tagname: "desktop-search",
      dataset: {
        active: false,
        closeonfocus: true,
        ignore: "search",
        innerHTML: `<input type="text" data-type="desktopsearch" data-ignore="search"></input>`
      }
    }),
    Util.create({
      tagname: "desktop-hub",
      ...
      innerHTML: `div class="hub-title">
            <div class="settings-button" id="desktop-hub-settings"><i class='bx bxs-cog bx-lg' ></i></div>
            <div class="title"><span>Hub</span></div>
        </div>
        <div class="topright">
            <div id="desktop-hub-close" class="closebtn"><i class='bx bx-x bx-md' ></i></div>
        </div>
        <div class="window-group"></div>
...
document.addEventListener("mousedown", (e) => {
  ...
  function dragSelect(e) {
    ...
    dragSelector.style.height =
      (Math.max(origin[1], e.clientY) - Math.min(origin[1], e.clientY)) + "px";
    dragSelector.style.width =
      (Math.max(origin[0], e.clientX) - Math.min(origin[0], e.clientX)) + "px";
  }
...
function openSignup(event) {
  ...
  function sendUsername(event) {
    setTimeout(() => {
      Server.Auth({
        req: "signupCheckUsernameAvailable",
  ...
  function sendEmail(event) {
    setTimeout(() => {
      Server.Auth({
        req: "signupCheckEmailRegistered",
        data: event.target.value,
      });
    }, 1);
    sendSignup(event);
  ...
  function sendSignup(event) {
    if (event.key == "Enter") {
      Server.Auth({
        ...
        data: {
          username:
            event.target.parentNode.parentNode.querySelectorAll("input")[0]
              .value,
          email: event.target.parentNode.parentNode.querySelectorAll("input")[1]
            .value,
          password:
            event.target.parentNode.parentNode.querySelectorAll("input")[2]
              .value,
          passwordconfirm:
            event.target.parentNode.parentNode.querySelectorAll("input")[3]
              .value,
        },
      });
    }
  ...
  function sendSignupBtn(event) {
    ...
    Server.Auth({
      ...
      data: {
        ...
        email: event.target.parentNode.parentNode.parentNode.querySelectorAll(
          "input",
        )[1].value,
        password:
          event.target.parentNode.parentNode.parentNode.querySelectorAll(
  ...
  function legalUsername(event) {
    ...
    if (match == null) {
      handlers["A-0009"]({ code: "A-0009", message: "Invalid username" });
    }
  }
                ...
                childElements: [
                  ...
                  },
                  {
                    ...
                    childElements: [
                      ...
                      {
                        tagname: "div",
                        innerHTML: "more than 8 characters long",
                        classList: ["pw-requirement"],
                        dataset: { met: false },
                      },
                      {
                        tagname: "div",
                        innerHTML: "and fulfill 3 of these requirements:",
                      },
                      {
                        tagname: "div",
                        innerHTML: "Contain uppercase letters",
                        classList: ["pw-requirement"],
                        dataset: { met: false },
                      },
                      {
                        tagname: "div",
                        innerHTML: "Contain lowercase letters",
                        classList: ["pw-requirement"],
                        dataset: { met: false },
                      },
                      {
                        tagname: "div",
                        innerHTML: "Contain numbers",
                        classList: ["pw-requirement"],
                        dataset: { met: false },
                      },
                      {
                        tagname: "div",
                        innerHTML: "Contain special characters",
                        classList: ["pw-requirement"],
                        dataset: { met: false },
                      },
                    ],
...
function loseFocus(event) {
  ...
  for (let e of elements) {
    if (
      event.target.dataset.ignore == "startmenu" && e.tagName == "START-MENU"
    ) {
      continue;
    }
    if (
      event.target.dataset.ignore == "search" && e.tagName == "DESKTOP-SEARCH"
    ) continue;
